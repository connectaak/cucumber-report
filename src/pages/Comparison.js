...
const Comparison = () => {
    ...
    const handleChangeDropDown = (event) => {
     ...
     if(selectedValues.find(item=>item.id==="select-all")){
      const selectedObjects = selectedValues.find(item=>item.id==="select-all");
    ...
    return (
       <Fragment>
        ...
        >
           <MenuItem key="select-all" value={{id:"select-all",name:"select-all",data:objectList}}>
            <CheckCircle
              sx={{
                visibility: selectedObjects.length === objectList.length ? 'visible' : 'hidden',
                marginRight: '8px',
              }}
            />
           All
