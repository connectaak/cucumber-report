...
const PichartCart = ({title,data}) => {
    ...
    const total=parseInt(data?.reduce((accumulator, currentValue) => accumulator + currentValue?.value, 0));
  ...
  const failed = parseInt(data.find((item) => item.name === "failed")?.value);
    ...
    const othersPercentage = Math.round(100 - (successfulPercentage + failedPercentage));
    ...
    return (
      ...
      inputProps={{ 'aria-label': 'controlled' }} />
        ...
        <CardContent>
          ...
          <Box className={checked === true ? classes.chartContainer : classes.chartContainerColumn}>
