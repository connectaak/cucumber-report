...
const CompareTable = ({ data: tableData }) => {
  ...
  return (
    <Paper
      sx={{ margin: "20px", boxShadow: "0px 4px 16px rgba(0, 0, 0, 0.3)" }}
    >
      <TableContainer>
        <Table
        ...
        >
          <TableHead>
            ...
            <TableRow>
              {header.map((item) => (
                <TableCell
                  className={`${classes.border} ${classes.width}`}
                  sx={{ bgcolor: COLORS[item] }}
                  align="center"
                >
                  {" "}
                  {item}
                ...
                {item.map((row, index) => (
                  <TableRow>
                    ...
                    <TableCell
                      className={`${classes.border} ${classes.width}`}
                      sx={{
                        bgcolor: !row.stepsPassed == 0 ? COLORS["Passed"] : "",
                      }}
                      align="center"
                    >
                      {row.stepsPassed}
                    ...
                    <TableCell
                      className={`${classes.border} ${classes.width}`}
                      sx={{
                        bgcolor: !row.stepsFailed == 0 ? COLORS["Failed"] : "",
                      }}
                      align="center"
                    >
                      {row.stepsFailed}
                    ...
                    <TableCell
                      className={`${classes.border} ${classes.width}`}
                      sx={{
                        bgcolor: !row.stepsTotal == 0 ? COLORS["Total"] : "",
                      }}
                      align="center"
                    >
                      {row.stepsTotal}
